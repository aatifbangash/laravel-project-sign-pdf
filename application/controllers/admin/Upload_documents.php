...
class Upload_documents extends My_Controller
{
				...
				if (!empty($_FILES['pdf_files']['name']) && sizeof($_FILES['pdf_files']['name']) > 0) {
					...
					if (!empty($totalFilesUploaded['error'])) {
						...
						if (!empty($totalFilesUploaded) && sizeof($totalFilesUploaded) > 0) {
							...
							if ($result) {
								// send email
								$captureRoleDetails = $this->upload_documents_model->getCaptureRoleById($this->input->post('capture_role'));

								$message = "Hello [name]!\r\nYou have been assigned a document that requires an esignature.\r\n\r\n[Linkhere]\r\n\r\nPlease review.\r\n\r\nThank you!\r\n[admin-email]";
								$message = str_replace('[name]', $captureRoleDetails->firstname, $message);
								$message = str_replace('[website]', 'esignaturepdf.com', $message);
								$message = str_replace('[admin-email]', 'johnsmith@gmail.com', $message);
								$message = str_replace('[Linkhere]', base_url('admin/sign_documents/list_files/' . $folderId * 786), $message);
								...
								$this->email->to($captureRoleDetails->email); // change it to yours
								$this->email->subject('You have been assigned a new document.');
	...
	public function sendmail()
	{
		...
		$config = array(
			'protocol' => 'smtp',
			'smtp_host' => 'ssl://nwpro3.fcomet.com',
			'smtp_port' => 465,
			'smtp_user' => 'info@esignaturepdf.com', // change it to yours
			'smtp_pass' => 'eSig@PDF@787', // change it to yours
			'mailtype' => 'html',
			'charset' => 'iso-8859-1',
			'wordwrap' => TRUE
		);
		...
		$this->email->from($from_email, $from_name); // change it to yours
		$this->email->to('atif@codenterprise.com'); // change it to yours
		$this->email->subject('Account has been created');
		$this->email->message($message);
		if ($this->email->send()) {
			echo 'Email sent.';
		} else {
